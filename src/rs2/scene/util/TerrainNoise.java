 ...
 *
 * <p>The arithmetic, constants, cosine interpolation, octave weights and final
 * clamp are preserved exactly. These helpers intentionally use integer
 * truncation and the client renderer's fixed-point cosine table.</p>
 */
public final class TerrainNoise {
    ...
    /** Returns the client's deterministic 0..255 pseudo-random value for a lattice point. */
    ...
    public static int smoothNoise(int x, int y) {
        int corners = randomNoise(x - 1, y - 1)
                + randomNoise(x + 1, y - 1)
                + randomNoise(x - 1, y + 1)
                + randomNoise(x + 1, y + 1);
        int sides = randomNoise(x - 1, y)
                + randomNoise(x + 1, y)
                + randomNoise(x, y - 1)
                + randomNoise(x, y + 1);
    ...
    /** Cosine-interpolates two samples using the renderer's 16-bit fixed-point cosine table. */
    ...
    /**
     * Produces the default plane-0 tile height before the map stream's factor-of-eight conversion.
     */
