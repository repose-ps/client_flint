...
public class Region {
    ...
    private static Renderable createRenderable(GameObjectDefinition definition, int objectId, int type,
                                               int orientation, int southWestHeight, int southEastHeight,
                                               int northEastHeight, int northWestHeight) {
        if (definition.animationId == -1 && definition.morphIds == null) {
            return definition.getModelAt(type, orientation, southWestHeight, southEastHeight,
                    northEastHeight, northWestHeight, -1);
        }
        return new DynamicObject(objectId, type, orientation,
     ...
     *
     * <p>This is intentionally separate from map-build placement: live updates do not
     * modify the region shadow/occlusion work arrays or minimum-plane state.</p>
     */
    public static void addLocation(int objectId, int heightPlane, int type, int orientation,
        ...
        if (type == 10 || type == 11) {
            Renderable renderable = createRenderable(definition, objectId, 10, orientation,
                    southWestHeight, southEastHeight, northEastHeight, northWestHeight);
            if (renderable != null) {
        ...
        if (type == 2) {
            ...
            Renderable primary = createRenderable(definition, objectId, 2, orientation + 4,
                    southWestHeight, southEastHeight, northEastHeight, northWestHeight);
    ...
    /** Clears one 8x8 instanced terrain chunk while preserving neighboring edge heights. */
    ...
    private void calculateTileLightness(int plane) {
        ...
        for (int y = 1; y < height - 1; y++) {
            for (int x = 1; x < width - 1; x++) {
                ...
                int shadow = (planeShadows[x - 1][y] >> 2)
                        + (planeShadows[x + 1][y] >> 3)
                        + (planeShadows[x][y - 1] >> 2)
                        + (planeShadows[x][y + 1] >> 3)
                        + (planeShadows[x][y] >> 1);
        ...
        for (int x = -5; x < width + 5; x++) {
            ...
            for (int y = -5; y < height + 5; y++) {
                ...
                if (underlayId > 0) {
                    ...
                    randomizedUnderlayHsl = FloorDefinition.packHsl(
                            randomizedHue, saturation, randomizedLightness);
                }
                ...
                if (plane > 0) {
                    ...
                    if (occludable && southWestHeight == southEastHeight
                ...
                if (textureId >= 0) {
                    ...
                    overlayHsl = -2;
                    textureId = -1;
                    overlayRgb = Rasterizer3D.HSL_TO_RGB[
                ...
                scene.addTile(plane, x, y, shape, overlayRotation, textureId,
                        ...
                        adjustOverlayLightness(overlayHsl, northWestLight),
                        underlayRgb, overlayRgb);
            }
        }
    }
    ...
    private boolean shouldBuildTile(int plane, int x, int y) {
        return !lowMemory
                || (tileFlags[0][x][y] & 0x2) != 0
                || ((tileFlags[plane][x][y] & 0x10) == 0
    ...
    /** Converts accumulated per-tile occlusion bits into the scene controller's merged clusters. */
    ...
    public void loadTerrainChunk(byte[] data, int sourcePlane, int sourceX, int sourceY,
                                 int destinationPlane, int destinationX, int destinationY,
                                 int rotation, CollisionMap[] collisionMaps) {
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) {
        ...
        for (int plane = 0; plane < 4; plane++) {
            for (int x = 0; x < 64; x++) {
                for (int y = 0; y < 64; y++) {
                    if (plane == sourcePlane && x >= sourceX && x < sourceX + 8
                            && y >= sourceY && y < sourceY + 8) {
                        int targetX = destinationX
                                + TiledUtils.getRotatedMapChunkX(x & 7, y & 7, rotation);
                        int targetY = destinationY
                                + TiledUtils.getRotatedMapChunkY(x & 7, y & 7, rotation);
        ...
        for (;;) {
            ...
            for (;;) {
                ...
                if (plane != sourcePlane || localX < sourceX || localX >= sourceX + 8
                        || localY < sourceY || localY >= sourceY + 8) {
                    continue;
                }

                GameObjectDefinition definition = GameObjectDefinition.lookup(objectId);
                int x = destinationX + TiledUtils.getRotatedLandscapeChunkX(
                        localX & 7, localY & 7, definition.sizeX, definition.sizeY,
                ...
                placeLocation(objectId, type, orientation + rotation & 0x3,
                        destinationPlane, x, y, collisionMap, scene);
            }
        }
    }
            ...
            if (renderable != null) {
                ...
                if (inserted && definition.castsShadow) {
                    Model shadowModel = renderable instanceof Model
                            ? (Model) renderable
                            : definition.getModelAt(10, orientation, southWestHeight, southEastHeight,
                                    northEastHeight, northWestHeight, -1);
                    if (shadowModel != null) {
    ...
    /** Fills an unavailable terrain rectangle with shadow 127 and copied edge heights. */
    ...
    /**
     * Scans a landscape stream and verifies the first relevant placement of each object has its models loaded.
     */
    ...
    /**
     * Decodes one terrain-tile record. Out-of-bounds targets still consume the complete record.
     */
    private void decodeTile(Buffer buffer, int plane, int x, int y,
                            int noiseX, int noiseY, int rotation) {
        if (x >= 0 && x < 104 && y >= 0 && y < 104) {
            ...
            for (;;) {
                ...
                if (opcode == 0) {
                    if (plane == 0) {
                        tileHeights[0][x][y] = -TerrainNoise.calculateHeight(932731 + x + noiseX,
                                556238 + y + noiseY) * 8;
                    } else {
