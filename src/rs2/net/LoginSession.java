 ...
 *
 * <p>The session owns retry counters, the server session key, and the temporary
 ...
 * {@link StatusSink} so protocol code does not depend on UI classes.</p>
 */
public final class LoginSession {
    ...
    /**
     * Builds, RSA-encrypts, and sends the credential/login block after response zero.
     *
     * @param username login username
     * @param password login password
    ...
    private boolean setKnownFailureMessage(int responseCode) {
        return switch (responseCode) {
            ...
            case 17 -> message("You are standing in a members-only area.",
