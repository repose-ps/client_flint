 ...
 *
 * <p>Glyph masks are read from {@code <name>.dat}; their dimensions and storage
 ...
 * right/bottom clipping quirk in the low-level glyph blitters.</p>
 *
 * <p>Formatted text uses the classic five-character tags such as
 * {@code @red@}, {@code @str@}, and {@code @end@}. Tags are recognized only by
 * methods whose names explicitly mention formatting.</p>
 */
public class TypeFace extends Rasterizer {
     ...
     *        {@code 'I'}; otherwise it is copied from {@code 'i'}. This odd rule
     *        is present in the original revision-377 client.
     * @param archive media archive containing the font data and {@code index.dat}
     * @param name font resource name without the {@code .dat} suffix
     */
    ...
    /** Returns the advance width while ignoring every five-character {@code @xxx@} span. */
    ...
    public void drawWaveText(String text, int centerX, int y, int color, int phase) {
        ...
        for (int index = 0; index < text.length(); index++) {
            ...
            if (character != ' ') {
                ...
                drawGlyph(glyphMasks[character], x + glyphXOffsets[character],
                        drawY + glyphYOffsets[character] + waveY, glyphWidths[character], glyphHeights[character], color);
            }
            x += glyphAdvances[character];
        }
    }
    ...
    /**
     * Draws centered text with the revision-377 decaying wave effect.
     * The amplitude is {@code max(0, 7 - age / 8)}.
     */
    ...
    /**
     * Draws text with classic {@code @xxx@} color/strikethrough tags.
     * Unknown tags are consumed exactly like recognized tags but do not alter state.
     */
    public void drawTextWithTags(String text, int x, int y, int color, boolean shadow) {
        ...
        if (strikethrough) {
            Rasterizer.drawHorizontalLine(startX, drawY + (int) (lineHeight * 0.7D), x - startX,
                    STRIKETHROUGH_COLOR);
        }
    }
     ...
     *
     * <p>The random source is deterministically reseeded for each call. Alpha is
     * always 192..223 inclusive, and one extra horizontal pixel is inserted with
     * probability 1/4 after each visible/text-space character.</p>
     */
    ...
    private int parseTag(String tag) {
        if (tag.equals("red")) return 0xff0000;
        if (tag.equals("gre")) return 0x00ff00;
        if (tag.equals("blu")) return 0x0000ff;
        if (tag.equals("yel")) return 0xffff00;
        if (tag.equals("cya")) return 0x00ffff;
        if (tag.equals("mag")) return 0xff00ff;
        if (tag.equals("whi")) return 0xffffff;
        if (tag.equals("bla")) return 0x000000;
        if (tag.equals("lre")) return 0xff9040;
        if (tag.equals("dre")) return 0x800000;
        if (tag.equals("dbl")) return 0x000080;
        if (tag.equals("or1")) return 0xffb000;
        if (tag.equals("or2")) return 0xff7000;
        if (tag.equals("or3")) return 0xff3000;
        if (tag.equals("gr1")) return 0xc0ff00;
        if (tag.equals("gr2")) return 0x80ff00;
        if (tag.equals("gr3")) return 0x40ff00;
        if (tag.equals("str")) strikethrough = true;
        if (tag.equals("end")) strikethrough = false;
        return -1;
    }
    ...
    private static void drawGlyphOpaque(int[] destinationPixels, byte[] sourceMask, int color, int source,
        ...
        for (int row = -height; row < 0; row++) {
            for (int group = groupsOfFour; group < 0; group++) {
                if (sourceMask[source++] != 0) destinationPixels[destination++] = color; else destination++;
                if (sourceMask[source++] != 0) destinationPixels[destination++] = color; else destination++;
                if (sourceMask[source++] != 0) destinationPixels[destination++] = color; else destination++;
                if (sourceMask[source++] != 0) destinationPixels[destination++] = color; else destination++;
            }
    ...
    private static void drawGlyphAlphaPixels(int source, int destinationRowSkip, int sourceRowSkip, int destination,
        ...
        int sourceColor = (((color & 0xff00ff) * alpha & 0xff00ff00)
                + ((color & 0xff00) * alpha & 0xff0000)) >> 8;
