...
/**
 * Shared movement, orientation, animation, overhead-text and hit state for players and NPCs.
 *
 * <p>Path entries are tile coordinates. {@link #pathLength} is capped at nine queued steps,
 * leaving element zero as the newest destination. Fine world coordinates use the classic
 * 128-units-per-tile scale and are centered using {@link #size}.</p>
 */
public abstract class Actor extends Renderable {
    ...
    public int y;
    public int rotation;
    /** Number of queued path steps that remained when the current action sequence began. */
    ...
    /** Movement delay accumulated when animation precedence prevents consuming the path. */
    ...
    /** Clears queued movement without changing the actor's current tile or fine coordinates. */
    ...
    /**
     * Base visibility hook. Concrete actor types report whether enough definition/appearance
     * state is available to render them.
     */
     ...
     *
     * @param running whether this step uses the doubled running movement rate
     * @param direction 0=NW, 1=N, 2=NE, 3=W, 4=E, 5=SW, 6=S, 7=SE
     */
    ...
    /** Stores one of the four timed hit-splat slots, using the first slot whose expiry has passed. */
     ...
     *
     * <p>Non-teleport moves within eight tiles are queued. Larger moves, or explicit teleports,
     * discard the path and immediately recompute fine world coordinates.</p>
     */
