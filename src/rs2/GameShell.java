 ...
 *
 * <p>The revision-377 client historically inherited from {@code Applet}. This
 ...
 * encoding and frame-coordinate adjustments remain compatible with the
 * original client.</p>
 */
public class GameShell extends Canvas
    ...
    public void run() {
        ...
        while (shutdownCountdown >= 0) {
            ...
            if (timingSamples[timingIndex] == 0L) {
                ...
                ratio = (int) ((long) (2560 * cycleDurationMillis)
                        / (currentTime - timingSamples[timingIndex]));
            }
            ...
            if (ratio > 256) {
                ratio = 256;
                sleepMillis = (int) ((long) cycleDurationMillis
                        - (currentTime - timingSamples[timingIndex]) / 10L);
            }
            ...
            if (debugTiming) {
                ...
                System.out.println("del:" + sleepMillis + " deltime:" + cycleDurationMillis
                        + " mindel:" + minimumSleepMillis);
    ...
    /**
     * Requests shutdown and retains the original ten-second fallback if the
     * game thread fails to observe the request.
     */
    ...
    /** Starts a client worker thread with the original start-then-prioritize ordering. */
