...
public final class ObjectActionHandler implements ClientActionDispatcher.ActionHandler {
	...
	public boolean dispatch(int actionId, MenuEntry entry) {
		...
		if (actionId == MenuState.OBJECT_OPTION_5) {
			...
			packets.objectOption5(objectId(argument0), argument1 + regionManager.baseX,
					argument2 + regionManager.baseY);
		}
		if (actionId == MenuState.OBJECT_OPTION_1) {
			walkToGameObject(argument2, argument1, argument0);
			packets.objectOption1(objectId(argument0), argument1 + regionManager.baseX,
					argument2 + regionManager.baseY);
		}
		if (actionId == MenuState.OBJECT_OPTION_3) {
			walkToGameObject(argument2, argument1, argument0);
			packets.objectOption3(objectId(argument0), argument1 + regionManager.baseX,
					argument2 + regionManager.baseY);
		}
		...
		if (actionId == MenuState.OBJECT_OPTION_4) {
			walkToGameObject(argument2, argument1, argument0);
			packets.objectOption4(objectId(argument0), argument1 + regionManager.baseX,
					argument2 + regionManager.baseY);
		}
		if (actionId == MenuState.OBJECT_OPTION_2) {
			walkToGameObject(argument2, argument1, argument0);
			packets.objectOption2(objectId(argument0), argument1 + regionManager.baseX,
					argument2 + regionManager.baseY);
		}
		return false;
	}
