...
/**
 * Applies revision-377 social, message, report, trade, and challenge menu
 * actions.
 */
public final class SocialActionHandler implements ClientActionDispatcher.ActionHandler {
	 ...
	 */
	public SocialActionHandler(SocialManager socialManager, ActorSynchronizer actors, ActionPacketEncoder packets,
			InterfaceController interfaces, GameRenderer gameRenderer, ChatController chatController,
			ClientActionDispatcher.Movement movement, SocialManager.MessageSink messages,
		...
		if (actionId == MenuState.REPORT_ABUSE) {
			...
			if (markerIndex3 != -1) {
				if (interfaces.state().openInterfaceId == -1) {
					...
					interfaces.state().reportAbuseInterfaceId = interfaces
