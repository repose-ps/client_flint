...
/**
 * Revision-377 MIDI selection, on-demand request, fade, volume and resume state.
 *
 * <p>Legacy ownership moved from client fields anInt1327 (selected track),
 * anInt1270 (requested track), aBoolean1271 (fade request), anInt1128
 * (resume delay), and aBoolean1266 (music enabled).</p>
 */
public final class MusicController {
    ...
    private static final class SignlinkMidiBackend implements MidiBackend {
        public void setVolume(int volume, boolean adjustPlayingTrack) {
            ...
            if (adjustPlayingTrack) Signlink.midi = "voladjust";
        }
        public void stop() {
    ...
    public MusicController() { this(new SignlinkMidiBackend()); }
    MusicController(MidiBackend backend) { this.backend = backend; }
    ...
    public boolean acceptOnDemandRequest(OnDemandRequest request) {
        if (request.type == OnDemandFetcher.MIDI && request.id == requestedTrackId && request.buffer != null) {
            if (enabled) backend.save(request.buffer, request.buffer.length, fadeRequestedTrack);
            return true;
        }
        return false;
    }
    ...
    public void applySetting(int setting, boolean lowMemory, Requester requester) {
        ...
        if (setting == 0) { backend.setVolume(0, enabled); enabled = true; }
        if (setting == 1) { backend.setVolume(-400, enabled); enabled = true; }
        if (setting == 2) { backend.setVolume(-800, enabled); enabled = true; }
        if (setting == 3) { backend.setVolume(-1200, enabled); enabled = true; }
    ...
    boolean fadeRequestedTrack() { return fadeRequestedTrack; }
    int requestedTrackId() { return requestedTrackId; }
    int selectedTrackId() { return selectedTrackId; }
    int resumeDelay() { return resumeDelay; }
}