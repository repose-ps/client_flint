...
public final class SoundEffectQueue {
    ...
    private static final class SignlinkWaveBackend implements WaveBackend {
        public boolean save(byte[] data, int length) { return Signlink.saveWave(data, length); }
        public boolean replay() { return Signlink.replayWave(); }
        public boolean reportErrors() { return Signlink.reportErrors; }
        public void setVolume(int volume) { Signlink.waveVolume = volume; }
    }
    ...
    public void applySetting(int setting) {
        if (setting == 0) { enabled = true; waveBackend.setVolume(0); }
        if (setting == 1) { enabled = true; waveBackend.setVolume(-400); }
        if (setting == 2) { enabled = true; waveBackend.setVolume(-800); }
        if (setting == 3) { enabled = true; waveBackend.setVolume(-1200); }
        if (setting == 4) enabled = false;
    }
    ...
    public void queueAreaSound(int soundId, int loopCount, int radius, int tileX, int tileY,
            int playerTileX, int playerTileY, boolean lowMemory) {
        if (playerTileX >= tileX - radius && playerTileX <= tileX + radius
                && playerTileY >= tileY - radius && playerTileY <= tileY + radius
                && enabled && !lowMemory && count < CAPACITY) {
            soundIds[count] = soundId;
            loopCounts[count] = loopCount;
    ...
    /*
     * Legacy client.method152(int i), sound-effect portion:
     *   i -> removed fixed -23763 sentinel; the only caller supplied -23763.
     *
     * Legacy queue fields moved here:
     *   anInt1035      -> count
     *   anIntArray1090 -> soundIds
     *   anIntArray1321 -> loopCounts
     *   anIntArray1259 -> delays
     *   anInt1272      -> lastPlayedSoundId
     *   anInt935       -> lastPlayedLoopCount
     *   anInt1179      -> lastWaveLength
            ...
            if (delays[index] <= 0) {
                ...
                try {
                    if (soundIds[index] == lastPlayedSoundId && loopCounts[index] == lastPlayedLoopCount) {
                        if (!waveBackend.replay()) retry = true;
                    } else {
                        /*
                        ...
                        if (clock.getAsLong() + (long) (data.position / 22)
                                > lastWaveStartTime + (long) (lastWaveLength / 22)) {
                ...
                if (!retry || delays[index] == RETRY_DELAY) remove(index--);
                else delays[index] = RETRY_DELAY;
            } else {
                delays[index]--;
            }
        }
    ...
    int soundId(int index) { return soundIds[index]; }
    int loopCount(int index) { return loopCounts[index]; }
    int delay(int index) { return delays[index]; }
    int lastPlayedSoundId() { return lastPlayedSoundId; }
    int lastPlayedLoopCount() { return lastPlayedLoopCount; }
}