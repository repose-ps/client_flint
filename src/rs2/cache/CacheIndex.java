 ...
 *
 * <p>Each six-byte index entry stores a three-byte file length and a
 * three-byte first-sector number. File data is split across 520-byte sectors.
 * Every sector contains an eight-byte header followed by at most 512 bytes of
 * payload.</p>
 *
 * <p>This object does not own either {@link RandomAccessFile}; their lifecycle
 * remains with the cache bootstrap code that opened them.</p>
 */
public final class CacheIndex {
    ...
    private static final int SECTOR_PAYLOAD_SIZE =
        SECTOR_SIZE - SECTOR_HEADER_SIZE;
     ...
     */
    private static final int MAX_MEDIUM_VALUE =
        0xffffff;
     ...
     *
     * <p>Locking that shared object coordinates operations across different
     * CacheIndex instances, not merely different threads using one instance.</p>
     */
    ...
    public CacheIndex(
        ...
        if (
            indexId < 0
                || indexId > MAX_INDEX_ID
        ) {
            throw new IllegalArgumentException(
                "indexId must be between 0 and 255: "
                    + indexId
            );
        }
        ...
        if (
            maxEntrySize < 0
                || maxEntrySize > MAX_MEDIUM_VALUE
        ) {
            throw new IllegalArgumentException(
                "Invalid maximum entry size: "
                    + maxEntrySize
            );
        }
        ...
        /*
         * Multiple indexes created with the same RandomAccessFile will share
         * this monitor.
         */
        ioLock = dataFile;
    }
    ...
    public byte[] read(int entryId) {
        ...
        synchronized (ioLock) {
            try {
                ...
                /*
                 * The original client treats unreadable cache entries as
                 * misses and attempts to download them again.
                 */
                return null;
            }
        }
     ...
     *
     * <p>The implementation first attempts to reuse the entry's existing
     * sector chain. If that chain does not exist or cannot be reused, a new
     * chain is appended to the data file.</p>
     *
    ...
    public boolean write(
        ...
        synchronized (ioLock) {
            try {
                /*
                 * Empty entries require no sector. The original implementation
                 * produced an unreadable empty entry because it still required
                 * a positive first-sector number.
                 */
                if (data.length == 0) {
                    writeIndexEntry(
                        entryId,
    ...
    private byte[] readEntry(
        ...
        while (destinationOffset < length) {
            /*
             * A zero sector is the end-of-chain marker. Encountering it before
             * all requested bytes have been read means the chain is truncated.
             */
            if (sector == 0) {
                return null;
            }
            ...
            int storedEntryId =
                readUnsignedShort(scratch, 0);

            int storedChunk =
                readUnsignedShort(scratch, 2);

            int nextSector =
                readMedium(scratch, 4);

            int storedIndexId =
                scratch[7] & 0xff;

            /*
             * These values prevent an invalid chain from crossing into another
             * file, chunk, or cache index.
             */
            if (
    ...
    private boolean writeEntry(
        ...
        while (sourceOffset < data.length) {
            ...
            /*
             * When overwriting, inspect the existing sector header to locate
             * and validate the next sector in the chain.
             */
            if (overwrite) {
                dataFile.seek(
                    (long) sector * SECTOR_SIZE
                ...
                if (readFully(
                    ...
                    /*
                     * A self-referencing sector would cause the writer to
                     * overwrite the same sector repeatedly.
                     */
            ...
            /*
             * The existing chain ended or this is a new entry. Allocate the
             * next sector at the end of the data file.
             */
            if (nextSector == 0) {
                overwrite = false;
                ...
                /*
                 * Before the current sector has been written, the calculated
                 * end-of-file sector may equal it. Sector chains cannot point
                 * to themselves, so advance to the following sector.
                 */
                if (nextSector == sector) {
                    nextSector++;
                }
     ...
     */
    private boolean isExistingSector(
        int sector
    ) throws IOException {
        return sector > 0
            && sector <= MAX_MEDIUM_VALUE
            && (long) sector
                <= dataFile.length() / SECTOR_SIZE;
    }

    /**
     * Determines whether a sector can be represented in a three-byte pointer.
     */
    private static boolean isWritableSector(
        int sector
    ) {
        return sector > 0
            && sector <= MAX_MEDIUM_VALUE;
    ...
    private static int readUnsignedShort(
        ...
        return (
            (source[offset] & 0xff) << 8
        ) | (
            source[offset + 1] & 0xff
        );
    }

    private static int readMedium(
        byte[] source,
        int offset
    ) {
        return (
            (source[offset] & 0xff) << 16
        ) | (
    ...
    private static void checkEntryId(
        int entryId
    ) {
        if (
            entryId < 0
                || entryId > MAX_ENTRY_ID
        ) {
            throw new IllegalArgumentException(
                "entryId must be between 0 and 65535: "
                    + entryId
            );
        }
    }
