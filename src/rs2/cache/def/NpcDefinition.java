 ...
 *
 * <p>Definitions are retained in the original 20-entry rotating decode cache, while lit base
 * body models use a separate 30-entry LRU cache. Morph selection is controlled by either a
 * varbit or a varp, with the varbit taking precedence exactly as in the client.</p>
 */
public class NpcDefinition {
    ...
    public int[] modelIds;
    /** Value consumed by opcode 91. Its role is not established by the supplied client. */
    ...
    /** Value consumed by opcode 92. Its role is not established by the supplied client. */
    ...
    /** Value consumed by opcode 90. Its role is not established by the supplied client. */
    ...
    /**
     * Builds the head/dialogue model, or {@code null} when required model files are unavailable.
     */
    ...
    /** Returns whether the currently selected morph points at a valid NPC definition. */
    ...
    /**
     * Returns a lit animated body model. The shared scratch model behavior is retained exactly;
     * callers must not retain the returned transformed scratch model as an immutable instance.
     */
    public Model getAnimatedModel(int primaryFrameId, int secondaryFrameId, int[] interleaveOrder) {
        if (morphIds != null) {
            NpcDefinition transformed = transform();
            return transformed == null ? null : transformed.getAnimatedModel(primaryFrameId, secondaryFrameId, interleaveOrder);
        }
        ...
        animatedModel.replaceWithModel(baseModel, AnimationFrame.isNull(primaryFrameId) & AnimationFrame.isNull(secondaryFrameId));
