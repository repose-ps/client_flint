...
public final class SceneEntityRenderer {
    ...
    public void addPlayers(WorldState world, ActorSynchronizer actors, Player localPlayer, int plane,
            int cycle, boolean lowMemory, boolean localOnly) {
        ...
        for (int index = 0; index < count; index++) {
            ...
            if (player.attachedModel != null && cycle >= player.attachedModelStartCycle
                ...
                world.scene.addEntityBounds(plane,
                        player.attachedModelMinX, player.attachedModelMinY,
                        player.attachedModelMaxX, player.attachedModelMaxY,
                        player.x, player.y, player.tileHeight, player, player.rotation, uid);
                continue;
            }
