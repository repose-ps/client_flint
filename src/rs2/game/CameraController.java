...
public final class CameraController {
    ...
    public void updateFollow(Actor localPlayer, int[] keyStatus, WorldState world, int plane,
            int regionX, int regionY, int baseX, int baseY) {
        try {
            int targetX = localPlayer.x + followOffsetX;
            int targetY = localPlayer.y + followOffsetY;
            if (followTargetX - targetX < -500 || followTargetX - targetX > 500
                    || followTargetY - targetY < -500 || followTargetY - targetY > 500) {
    ...
    public void setCinematicPosition(int tileX, int tileY, int heightOffset, int baseSpeed, int scale,
            WorldState world, int plane) {
        cinematic = true;
        positionTileX = tileX;
        positionTileY = tileY;
    ...
    public void setCinematicLookAt(int tileX, int tileY, int heightOffset, int baseSpeed, int scale,
            WorldState world, int plane) {
        cinematic = true;
        lookTileX = tileX;
        lookTileY = tileY;
    ...
    public void tickRandomOffsets() {
        ...
        if (followOffsetX < -50) followOffsetXStep = 2;
        if (followOffsetX > 50) followOffsetXStep = -2;
        if (followOffsetY < -55) followOffsetYStep = 2;
        if (followOffsetY > 55) followOffsetYStep = -2;
        if (yawOffset < -40) yawOffsetStep = 1;
        if (yawOffset > 40) yawOffsetStep = -1;
    }
