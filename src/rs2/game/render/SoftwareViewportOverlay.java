 ...
 */
public record SoftwareViewportOverlay(int[] pixels, int sourceStride, int x, int y, int width, int height,
    ...
    public SoftwareViewportOverlay(int[] pixels, int sourceStride, int x, int y, int width, int height) {
        this(pixels, sourceStride, x, y, width, height, OPAQUE);
    }
    ...
    public SoftwareViewportOverlay {
        ...
        if (sourceStride <= 0 || x < 0 || y < 0 || width <= 0 || height <= 0) {
            throw new IllegalArgumentException("Invalid viewport overlay rectangle.");
        }
        long finalIndex = (long) (y + height - 1) * sourceStride + x + width;
        if (x + width > sourceStride || finalIndex > pixels.length) {
