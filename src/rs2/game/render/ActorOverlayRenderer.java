 ...
 *
 * <p>
 * The renderer owns the short-lived projection coordinates and the fixed
 ...
 * is supplied through a per-frame {@link Context}.
 * </p>
 */
		...
		for (int actorListIndex = -1; actorListIndex < actors.playerCount + actors.npcCount; actorListIndex++) {
			...
			if (actor.overheadText != null
					&& (actorListIndex >= actors.playerCount || context.publicChatMode == ChatMode.ON
							|| context.publicChatMode == ChatMode.HIDE || context.publicChatMode == ChatMode.FRIENDS
		...
		for (int overheadIndex = 0; overheadIndex < overheadTextCount; overheadIndex++) {
			...
			if (context.chatEffects == 0) {
				...
				if (effect == 2) {
					...
					context.boldFont.drawWave2Text(overheadText, projectedX, projectedY, textColor,
							context.renderCycle);
				}
				if (effect == 3) {
