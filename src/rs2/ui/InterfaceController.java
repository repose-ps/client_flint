 ...
 *
 * <p>
 * The controller deliberately does not render widgets or interpret menu
 ...
 * "please wait" interface action flag.
 * </p>
 */
public final class InterfaceController {
	...
	public void handleScrollbarInput(int scrollHeight, int y, Widget widget, int mouseY, int redrawArea, int mouseX,
		...
		scrollbarDragging = false;
		if (mouseX >= x && mouseX < x + ClientLayout.SCROLLBAR_WIDTH && mouseY >= y
		...
		if (mouseX >= x - scrollbarDragPadding && mouseX < x + ClientLayout.SCROLLBAR_WIDTH + scrollbarDragPadding
				&& mouseY >= y + ClientLayout.SCROLLBAR_ARROW_HEIGHT
