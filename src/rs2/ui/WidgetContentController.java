 ...
 *
 * <p>
 * The controller translates social, appearance, report-abuse, and account state
 * into the mutable widget fields consumed by {@link WidgetRenderer}. Rendering
 * remains responsible only for drawing the resulting widget state; the
 * top-level client is not exposed through a generic mutation callback.
 * </p>
 */
public final class WidgetContentController {
	 ...
	 * @param membersWorld           whether the current world enables members
	 *                               features
	 * @param lastLoginHost          resolved host text for the previous login
	 *                               address
	 */
	public record AccountStatus(int currentDay, int lastLoginDay, int recoveryQuestionsDate, int unreadMessageCount,
			int lastPasswordChangeDate, int membershipDays, boolean membersWorld, String lastLoginHost) {
	}
	...
	private static void updateAccountContent(Widget widget, int contentType, AccountStatus status) {
		...
		if (contentType == WidgetContentType.ACCOUNT_PASSWORD_CHANGE)
			if (status.lastPasswordChangeDate() <= 0 || status.lastPasswordChangeDate() > status.currentDay() + 10)
