 ...
 *
 * <p>
 * The renderer consumes interface interaction state from
...
public final class WidgetRenderer {
	 ...
	 */
	public record RenderContext(int mouseX, int mouseY, int animationCycleDelta, TypeFace smallFont, TypeFace plainFont,
			IndexedImage scrollbarTop, IndexedImage scrollbarBottom, int scrollbarTrackColor, int scrollbarThumbColor,
			int scrollbarHighlightColor, int scrollbarShadowColor) {
	}
	...
	public void drawScrollbar(RenderContext context, int scrollY, int x, int height, int scrollHeight, int y) {
		...
		Rasterizer.drawFilledRectangle(x, y + arrowHeight, scrollbarWidth, trackHeight, context.scrollbarTrackColor());
	...
	private void drawInterfaceRecursive(RenderContext context, int y, int x, Widget widget, int scrollY) {
		if (widget.type != Widget.TYPE_CONTAINER || widget.children == null)
			return;
		if (widget.mouseoverTriggered && interfaceController.viewportHoveredWidgetId() != widget.id
				&& interfaceController.sidebarHoveredWidgetId() != widget.id
		...
		for (int childIndex = 0; childIndex < childCount; childIndex++) {
			...
			if (childWidget.type == Widget.TYPE_CONTAINER) {
				...
				if (childWidget.type == Widget.TYPE_INVENTORY) {
						...
						for (int column = 0; column < childWidget.width; column++) {
							...
							if (childWidget.itemIds[slot] > 0) {
								...
								if (slotX > Rasterizer.topX - 32 && slotX < Rasterizer.bottomX
									...
									if (interfaceController.state().itemSelected == 1
											&& interfaceController.state().selectedItemSlot == slot
									...
									if (itemSprite != null) {
										if (interfaceController.state().inventoryDragArea != 0
												&& interfaceController.state().draggedInventorySlot == slot
												&& interfaceController
														.state().draggedInventoryWidgetId == childWidget.id) {
											dragOffsetX = context.mouseX()
													- interfaceController.state().inventoryDragStartX;
											dragOffsetY = context.mouseY()
													- interfaceController.state().inventoryDragStartY;
											...
											if (slotY + dragOffsetY + 32 > Rasterizer.bottomY
												...
												&& interfaceController
														.state().pressedInventoryWidgetId == childWidget.id)
										...
										if (itemSprite.maxWidth == 33 || childWidget.itemAmounts[slot] != 1) {
											...
											context.smallFont().drawText(formatItemStackAmount(itemAmount),
													slotX + dragOffsetX, slotY + 9 + dragOffsetY, 0xffff00);
										}
									}
								}
					...
					if (interfaceController.chatboxHoveredWidgetId() == childWidget.id
							|| interfaceController.sidebarHoveredWidgetId() == childWidget.id
					...
					if (childWidget.type == Widget.TYPE_TOOLTIP
							&& (interfaceController.chatboxTooltipWidgetId() == childWidget.id
									|| interfaceController.sidebarTooltipWidgetId() == childWidget.id
									|| interfaceController.viewportTooltipWidgetId() == childWidget.id)
