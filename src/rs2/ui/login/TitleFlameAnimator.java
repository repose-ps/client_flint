 ...
 * animation thread.
 *
 * <p>The animator preserves the revision-377 128-by-256 intensity simulation
 * and composites it onto the two fixed 128-by-265 title buffers. The game
 * cycle is supplied by the client so the original sine-wave line offsets remain
...
public final class TitleFlameAnimator {
    ...
    /** Stops the worker, waits for it to terminate, and releases title-only resources. */
    public void dispose() {
        running = false;
    ...
    private void initializePalettes() {
        ...
        for (int i = 0; i < 64; i++) redPalette[i] = i * 0x40000;
        for (int i = 0; i < 64; i++) redPalette[i + 64] = 0xff0000 + 1024 * i;
        for (int i = 0; i < 64; i++) redPalette[i + 128] = 0xffff00 + 4 * i;
        for (int i = 0; i < 64; i++) redPalette[i + 192] = 0xffffff;
        greenPalette = new int[PALETTE_SIZE];
        for (int i = 0; i < 64; i++) greenPalette[i] = i * 1024;
        for (int i = 0; i < 64; i++) greenPalette[i + 64] = 65280 + 4 * i;
        for (int i = 0; i < 64; i++) greenPalette[i + 128] = 65535 + 0x40000 * i;
        for (int i = 0; i < 64; i++) greenPalette[i + 192] = 0xffffff;
        bluePalette = new int[PALETTE_SIZE];
        for (int i = 0; i < 64; i++) bluePalette[i] = i * 4;
        for (int i = 0; i < 64; i++) bluePalette[i + 64] = 255 + 0x40000 * i;
        for (int i = 0; i < 64; i++) bluePalette[i + 128] = 0xff00ff + 1024 * i;
        for (int i = 0; i < 64; i++) bluePalette[i + 192] = 0xffffff;
    }
    ...
    private void update() {
        ...
        for (int blurY = 1; blurY < FLAME_HEIGHT - 1; blurY++) {
            for (int blurX = 1; blurX < 127; blurX++) {
                ...
                intensityScratch[offset] = (intensity[offset - 1] + intensity[offset + 1]
                        + intensity[offset - 128] + intensity[offset + 128]) / 4;
            }
        }
        noiseOffset += 128;
        ...
        for (int i = 0; i < FLAME_HEIGHT - 1; i++) lineOffsets[i] = lineOffsets[i + 1];
        ...
        if (greenTransition > 0) greenTransition -= 4;
        if (blueTransition > 0) blueTransition -= 4;
        if (greenTransition == 0 && blueTransition == 0) {
            int roll = (int) (Math.random() * 2000D);
            if (roll == 0) greenTransition = 1024;
            if (roll == 1) blueTransition = 1024;
        }
    }

    /**
     * Randomizes and smooths the flame noise field and applies an optional rune mask.
     * @param rune optional rune mask, or {@code null} for an unmasked field
     */
    private void initializeNoise(IndexedImage rune) {
        for (int i = 0; i < noise.length; i++) noise[i] = 0;
        for (int i = 0; i < 5000; i++) noise[(int) (Math.random() * SIMULATION_SIZE)] = (int) (Math.random() * 256D);
        for (int pass = 0; pass < 20; pass++) {
            for (int y = 1; y < FLAME_HEIGHT - 1; y++) {
                for (int x = 1; x < 127; x++) {
                    int offset = x + (y << 7);
                    noiseScratch[offset] = (noise[offset - 1] + noise[offset + 1]
                            + noise[offset - 128] + noise[offset + 128]) / 4;
                }
            }
            int[] swap = noise;
            noise = noiseScratch;
    ...
    private void draw() {
        if (greenTransition > 0) {
            for (int i = 0; i < PALETTE_SIZE; i++) {
                if (greenTransition > 768) palette[i] = blendColors(redPalette[i], greenPalette[i], 1024 - greenTransition);
                else if (greenTransition > 256) palette[i] = greenPalette[i];
                else palette[i] = blendColors(greenPalette[i], redPalette[i], 256 - greenTransition);
            }
        } else if (blueTransition > 0) {
            for (int i = 0; i < PALETTE_SIZE; i++) {
                if (blueTransition > 768) palette[i] = blendColors(redPalette[i], bluePalette[i], 1024 - blueTransition);
                else if (blueTransition > 256) palette[i] = bluePalette[i];
                else palette[i] = blendColors(bluePalette[i], redPalette[i], 256 - blueTransition);
            }
        } else {
            ...
            for (int column = leftInset; column < FLAME_WIDTH; column++) {
                ...
                if (value != 0) {
                    ...
                    leftBuffer.pixels[framebufferOffset++] = ((flameColor & 0xff00ff) * alpha
                            ...
                            + ((flameColor & 0xff00) * alpha + (backgroundColor & 0xff00) * inverseAlpha & 0xff0000) >> 8;
                } else framebufferOffset++;
            }
