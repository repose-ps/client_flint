 ...
 *
 * <p>
 * The cache-free cases are the permanent 23-check renderer gate. Supplying a
 * revision-377 cache directory adds the historical real-texture fixture.
 * </p>
 */
public final class RendererGoldenTest {
	...
	private static void testPlainSceneTileRasterization() {
		...
		int pitch = 128;
		scene.renderPlainTile(tile, 0, 0, 0, Rasterizer3D.SINE[pitch], Rasterizer3D.COSINE[pitch], Rasterizer3D.SINE[0],
				Rasterizer3D.COSINE[0]);
	...
	private static void testShapedSceneTileRasterization() {
		...
		initializeRaster(160, 120, background);
		Scene scene = createScene();
		ComplexTile tile = new ComplexTile(0, 0, 12, 24, 40, 0, 1, -1, 7, 0x1100, 0x1500, 0x2500, 0x2900, 0x3900,
				0x3d00, 0x4d00, 0x5100, 0, 0);
		setSceneCamera();

		int pitch = 128;
		scene.renderShapedTile(tile, 0, 0, Rasterizer3D.SINE[pitch], Rasterizer3D.COSINE[pitch], Rasterizer3D.SINE[0],
				Rasterizer3D.COSINE[0]);
	...
	private static void testRealCacheTextureRasterization(Path cache) throws Exception {
		...
		initializeRaster(96, 72, background);
		Rasterizer3D.restrictEdges = true;
		Rasterizer3D.drawTexturedTriangle(5, 66, 22, 8, 88, 44, 64, 96, 80, -64, 64, 0, -48, -48, 64, 256, 256, 320, 0);
	...
	private static void initializeRaster(int width, int height, int background) {
		...
		for (int index = 0; index < Rasterizer3D.HSL_TO_RGB.length; index++) {
			Rasterizer3D.HSL_TO_RGB[index] = ((index * 37) & 0xff) << 16 | ((index * 73) & 0xff) << 8
					| (index * 109) & 0xff;
		}
	}
