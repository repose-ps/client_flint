 ...
 *
 * <p>
 * The checks intentionally use reflection so they validate compiled dependency
 * surfaces without requiring a source-tree location at runtime.
 * </p>
 */
public final class ArchitectureSelfTest {
	 ...
	 */
	private static final String[] PACKET_HANDLER_CLASSES = { "rs2.packet.PacketDomainDispatcher",
			"rs2.packet.InterfacePacketHandler", "rs2.packet.SocialPacketHandler", "rs2.packet.RegionPacketHandler",
			"rs2.packet.CameraPacketHandler", "rs2.packet.AudioPacketHandler", "rs2.packet.ActorPacketHandler",
			"rs2.packet.ClientStatePacketHandler" };

	/** Cohesive menu-action effect handlers extracted during Phase 7.3. */
	private static final String[] ACTION_HANDLER_CLASSES = { "rs2.action.PlayerActionHandler",
			"rs2.action.NpcActionHandler", "rs2.action.ObjectActionHandler", "rs2.action.GroundItemActionHandler",
			"rs2.action.InventoryActionHandler", "rs2.action.WidgetActionHandler", "rs2.action.SocialActionHandler",
			"rs2.action.WalkActionHandler" };

	/** Classes moved during the post-Phase-6 package cohesion sweep. */
	private static final String[] PACKAGED_CLASSES = { "rs2.action.ClientActionDispatcher", "rs2.shell.GameShell",
			"rs2.shell.GameFrame", "rs2.ui.ClientLayout", "rs2.media.animation.AnimationFrame",
			"rs2.media.animation.Skeleton", "rs2.scene.tile.GroundItemTile", "rs2.scene.tile.InteractiveObject" };

	/** Obsolete pre-sweep class names that must not reappear. */
	private static final String[] OBSOLETE_CLASS_NAMES = { "rs2.ClientActionDispatcher",
			"rs2.ClientIncomingPacketHandler", "rs2.InterfacePacketHandler", "rs2.SocialPacketHandler",
			"rs2.RegionPacketHandler", "rs2.CameraPacketHandler", "rs2.AudioPacketHandler", "rs2.ActorPacketHandler",
			"rs2.ClientStatePacketHandler", "rs2.GameShell", "rs2.GameFrame", "rs2.ClientLayout",
	...
	private static final Set<String> REMOVED_CLIENT_BRIDGES = Set.of("packetSocialManager", "packetChatController",
			...
			"packetMinimapRenderer", "packetRegionManager", "packetVarpState", "packetWorldState", "packetZoneUpdates",
			"packetActorChatHandler", "requestSidebarRedraw", "requestChatboxRedraw", "requestTabAreaRedraw",
			"requestChatModesRedraw", "requestGameScreenRedraw", "queueAreaSound", "resetCharacterAppearance",
			"getConfiguredHost", "drawActorOverlays", "drawWorldHintIcon", "dispatchPlayerMenuAction",
			"dispatchNpcMenuAction", "dispatchObjectMenuAction", "dispatchGroundItemMenuAction",
			"dispatchInventoryMenuAction", "dispatchWidgetMenuAction", "dispatchSocialMenuAction",
			"dispatchMiscMenuAction", "walkToGameObject", "markInventoryInteraction", "handleWidgetContentAction",
			"dispatchMenuAction", "isAddFriendMenuAction");

	/**
	 * Action-adjacent Client operations that remain internal composition details.
	 */
	private static final Set<String> PRIVATE_CLIENT_ACTION_METHODS = Set.of("closeInterfaces", "addFriend", "addIgnore",
			"removeFriend", "removeIgnore");

	/** Ambiguous pre-6.3 Buffer method names that must not reappear. */
	private static final Set<String> LEGACY_BUFFER_METHODS = Set.of("writeLength", "startBitAccess", "readByteAdd",
			"readByteNeg", "readByteSub", "writeShortAddLE", "readUnsignedShortAddLE", "readShortLE", "readShortAdd");
	...
	/**
	 * Verifies that decomposition-era client state is no longer exposed as public
	 * fields.
	 *
	...
	/**
	 * Verifies that the sole root packet adapter is package-private and
	 * intentionally owns the Client boundary.
	 *
	 * @param test assertion sink
				...
				for (Constructor<?> constructor : handler.getDeclaredConstructors()) {
					...
					test.check(
							Arrays.stream(constructor.getParameterTypes())
									.noneMatch(type -> type == NetworkSession.class),
		...
		for (String className : ACTION_HANDLER_CLASSES) {
			try {
				...
				for (Constructor<?> constructor : handler.getDeclaredConstructors()) {
					test.check(
							Arrays.stream(constructor.getParameterTypes())
									.noneMatch(type -> type == MenuController.class),
	...
	private static void testActionPacketBoundary(SelfTestSupport test) {
		...
		long bufferFields = Arrays.stream(encoder.getDeclaredFields()).filter(field -> field.getType() == Buffer.class)
				.count();
	...
	/**
	 * Verifies that widget rendering no longer owns a generic application mutation
	 * callback.
	 *
	 * @param test assertion sink
	 */
	...
	/**
	 * Verifies that menu state retains cohesive entries instead of the old parallel
	 * arrays.
	 *
	 * @param test assertion sink
	 */
	...
	/**
	 * Verifies that ambiguous Buffer operation names removed in Phase 6.3 stay
	 * absent.
	 *
	 * @param test assertion sink
	 */
	private static void testBufferNaming(SelfTestSupport test) {
		Set<String> methodNames = Arrays.stream(Buffer.class.getDeclaredMethods()).map(Method::getName)
				.collect(java.util.stream.Collectors.toSet());
