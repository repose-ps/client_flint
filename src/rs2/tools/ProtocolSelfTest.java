...
public final class ProtocolSelfTest {
	...
	private static void testBufferWrites(SelfTestSupport test) {
		...
		test.bytes(Arrays.copyOf(buffer.payload, buffer.position),
				new byte[] { 0x12, 0x34, 0x56, (byte) 0x9a, 0x78, (byte) 0xbc, (byte) 0xde, (byte) 0xf0, 0x12, 0x34,
						0x56, 0x78, (byte) 0xef, (byte) 0xcd, (byte) 0xab, (byte) 0x90, 0x11, 0x22, 0x33, 0x44, 0x55,
						0x66, 0x77, (byte) 0x88, 'H', 'i', 0x0a, 8, 7 },
	...
	private static void testBufferReads(SelfTestSupport test) {
		Buffer buffer = new Buffer(new byte[] { (byte) 0xfe, 0x12, 0x34, (byte) 0xff, (byte) 0xfe, 0x01, 0x23, 0x45,
				(byte) 0x89, (byte) 0xab, (byte) 0xcd, (byte) 0xef, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
				(byte) 0x88, 'o', 'k', 0x0a, 4, 5, 6 });
	...
	private static void testBufferTransforms(SelfTestSupport test) {
		...
		Buffer unsignedBytes = new Buffer(new byte[] { (byte) 0x80, (byte) 0xff, 0x00, 0x7f, 0x00, (byte) 0x81,
				(byte) 0x80, 0x01, (byte) 0x80, 0x01, 0x00, (byte) 0x81 });
		...
		Buffer signedBytes = new Buffer(new byte[] { 0x00, 0x7f, (byte) 0x80, (byte) 0xff, (byte) 0x80, 0x01, 0x00,
				(byte) 0x81, 0x00, (byte) 0x81, (byte) 0x80, 0x01 });
		...
		Buffer multiByteReads = new Buffer(new byte[] { 0x34, 0x12, 0x12, (byte) 0xb4, (byte) 0xb4, 0x12, 0x34, 0x12,
				0x56, 0x78, 0x56, 0x34, 0x12, 0x56, 0x78, 0x12, 0x34, 0x34, 0x12, 0x78, 0x56 });
		...
		Buffer signedShorts = new Buffer(new byte[] { (byte) 0xfe, (byte) 0xff, (byte) 0xff, (byte) 0xff });
	...
	private static void testIsaacKnownVectors(SelfTestSupport test) {
		int[] zeroExpected = { 0x182600f3, 0x300b4a8d, 0x301b6622, 0xb08acd21, 0x296fd679, 0x995206e9, 0xb3ffa8b5,
				0x0fc99c24, 0x5f071faf, 0x52251def, 0x894f41c2, 0xcc4c9afb };
		int[] seededExpected = { 0xdaf8863e, 0x74a5cb37, 0xafd4ed73, 0x877c7c44, 0x8fc83d9b, 0x606024ad, 0xff7a07aa,
				0x4fb9c0c7, 0xd47d4e08, 0xea54103c, 0x0ac4e8c1, 0xfc624e56 };
	...
	private static void testMovementPacketEncoding(SelfTestSupport test) {
		...
		Pathfinder.Route route = new Pathfinder().findRoute(collisionMap, 50, 50, 55, 53, 0, 0, 0, 0, 0, false);
		...
		test.bytes(Arrays.copyOf(outgoing.payload, outgoing.position),
				new byte[] { 0x5a, 0x07, 0x34, 0x0c, 0x01, 0x32, 0x0c, 0x03, 0x7d }, "screen movement packet bytes");
	}

	/**
	 * Verifies fixed, variable-byte and variable-short packet framing over loopback
	 * TCP.
	 *
	 * @param test assertion sink
	...
	private static void testIncomingPacketFraming(SelfTestSupport test) throws Exception {
		byte[] wire = { 2, 1, 2, 3, 4, 63, 3, 5, 6, 7, 53, 0, 3, 8, 9, 10 };
	...
	/**
	 * Verifies handshake and outer credential/login framing against a loopback
	 * server.
	 *
	 * @param test assertion sink
	...
	private static void testLoginFraming(SelfTestSupport test) throws Exception {
		...
		try (ServerSocket server = new ServerSocket(0, 1, InetAddress.getLoopbackAddress())) {
			...
			NetworkSession network = new NetworkSession();
			LoginSession login = new LoginSession(network, port -> new Socket(server.getInetAddress(), port),
					index -> archiveCrcs[index], new SilentStatusSink(), listener, BigInteger.ONE,
					BigInteger.ONE.shiftLeft(1024).subtract(BigInteger.ONE), server.getLocalPort());
			try {
