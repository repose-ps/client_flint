 ...
 *
 * <p>
 * The suite freezes the Phase 7 action-domain boundary and verifies every
 ...
 * semantics and representative behavior of extracted concrete handlers.
 * </p>
 */
public final class ActionDispatchSelfTest {

	/** Non-action public integers on {@link MenuState}. */
	private static final Set<String> STRUCTURAL_CONSTANTS = Set.of("CAPACITY", "LOW_PRIORITY_OFFSET", "BUILD_LIMIT");
	...
	private static final Map<MenuActionDomain, int[]> ACTIONS = Map.ofEntries(
			Map.entry(MenuActionDomain.PLAYER,
					new int[] { MenuState.PLAYER_OPTION_1, MenuState.PLAYER_OPTION_2, MenuState.PLAYER_OPTION_3,
							MenuState.PLAYER_OPTION_4, MenuState.PLAYER_OPTION_5, MenuState.USE_ITEM_ON_PLAYER,
							MenuState.CAST_SPELL_ON_PLAYER }),
			Map.entry(MenuActionDomain.NPC,
					new int[] { MenuState.NPC_OPTION_1, MenuState.NPC_OPTION_2, MenuState.NPC_OPTION_3,
							MenuState.NPC_OPTION_4, MenuState.NPC_OPTION_5, MenuState.USE_ITEM_ON_NPC,
							MenuState.CAST_SPELL_ON_NPC, MenuState.EXAMINE_NPC }),
			Map.entry(MenuActionDomain.OBJECT,
					new int[] { MenuState.OBJECT_OPTION_1, MenuState.OBJECT_OPTION_2, MenuState.OBJECT_OPTION_3,
			...
			Map.entry(MenuActionDomain.GROUND_ITEM,
					new int[] { MenuState.GROUND_ITEM_OPTION_1, MenuState.GROUND_ITEM_OPTION_2,
			...
			Map.entry(MenuActionDomain.INVENTORY, new int[] { MenuState.INVENTORY_ITEM_OPTION_1,
					MenuState.INVENTORY_ITEM_OPTION_2, MenuState.INVENTORY_ITEM_OPTION_3,
					MenuState.INVENTORY_ITEM_OPTION_4, MenuState.INVENTORY_ITEM_OPTION_5, MenuState.SELECT_ITEM,
					MenuState.USE_ITEM_ON_INVENTORY_ITEM, MenuState.CAST_SPELL_ON_INVENTORY_ITEM,
					MenuState.EXAMINE_INVENTORY_ITEM, MenuState.WIDGET_ITEM_OPTION_1, MenuState.WIDGET_ITEM_OPTION_2,
					MenuState.WIDGET_ITEM_OPTION_3, MenuState.WIDGET_ITEM_OPTION_4, MenuState.WIDGET_ITEM_OPTION_5 }),
			Map.entry(MenuActionDomain.WIDGET,
					new int[] { MenuState.WIDGET_BUTTON, MenuState.SELECT_SPELL, MenuState.CLOSE_DIALOGUE,
							MenuState.CLOSE_INTERFACE, MenuState.WIDGET_TOGGLE_VARP, MenuState.WIDGET_SET_VARP,
							MenuState.WIDGET_CONTINUE }),
			Map.entry(MenuActionDomain.SOCIAL,
					new int[] { MenuState.ADD_FRIEND, MenuState.ADD_IGNORE, MenuState.REMOVE_FRIEND,
	...
	/**
	 * Verifies that every public action constant on {@link MenuState} is
	 * represented by the explicit census and no structural constant is mistaken for
	 * an action.
	 *
	 * @param test        assertion sink
	...
	/**
	 * Verifies unsupported and structural values cannot accidentally enter a
	 * domain.
	 * 
	 * @param test assertion sink
	 */
	...
	/**
	 * Exercises one concrete extracted target handler, including movement,
	 * crosshair, and transformed packet-field behavior.
	 *
	 * @param test assertion sink
	 */
	private static void testConcretePlayerHandler(SelfTestSupport test) {
		...
		PlayerActionHandler handler = new PlayerActionHandler(actors, new ActionPacketEncoder(network.outgoing),
				interfaces, (allowAlternative, targetX, targetY, targetWidth, targetHeight, movementType,
						interactionType, orientation, accessMask) -> {
					movement[0]++;
