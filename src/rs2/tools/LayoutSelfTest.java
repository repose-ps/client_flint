...
public final class LayoutSelfTest {
	...
	private static void testHitTesting(SelfTestSupport test) {
		...
		test.equal(fixed.chatModeButtonAt(6, 467), ClientLayout.CHAT_MODE_PUBLIC, "fixed public chat button");
		test.equal(fixed.chatModeButtonAt(135, 467), ClientLayout.CHAT_MODE_PRIVATE, "fixed private chat button");
		test.equal(fixed.chatModeButtonAt(273, 467), ClientLayout.CHAT_MODE_TRADE, "fixed trade chat button");
		test.equal(fixed.chatModeButtonAt(412, 467), ClientLayout.CHAT_MODE_REPORT_ABUSE, "fixed report-abuse button");
	...
	private static void testTabGeometry(SelfTestSupport test) {
		int[][] fixedBounds = { { 539, 574, 169, 205 }, { 569, 600, 168, 205 }, { 597, 628, 168, 205 },
				{ 625, 670, 168, 203 }, { 666, 697, 168, 205 }, { 694, 725, 168, 205 }, { 722, 757, 169, 205 },
				{ 540, 575, 466, 502 }, { 572, 603, 466, 503 }, { 599, 630, 466, 503 }, { 627, 672, 467, 502 },
				{ 669, 700, 466, 503 }, { 696, 727, 466, 503 }, { 724, 759, 466, 502 } };
	...
	private static void testChatModeGeometry(SelfTestSupport test) {
		int[][] fixedBounds = { { 6, 107, 467, 500 }, { 135, 236, 467, 500 }, { 273, 374, 467, 500 },
				{ 412, 513, 467, 500 } };
	...
	private static void testMinimapClickGeometry(SelfTestSupport test) {
		...
		ClientLayout fixed = new ClientLayout();
		MinimapRenderer.Click fixedCenter = minimap.transformClick(fixed.minimapLocalX(fixed.minimapX() + 98),
				fixed.minimapLocalY(fixed.minimapY() + 80), player, 0);
		...
		ClientLayout resized = new ClientLayout();
		resized.resize(1000, 700);
		MinimapRenderer.Click resizedCenter = minimap.transformClick(resized.minimapLocalX(resized.minimapX() + 98),
				resized.minimapLocalY(resized.minimapY() + 80), player, 0);
	...
	private static void testContextMenuGeometry(SelfTestSupport test) {
		...
		test.check(!layout.isContextMenuEntryHit(21, entryY - ClientLayout.CONTEXT_MENU_ENTRY_HIT_TOP, 20, 100, entryY),
				"context menu entry strict top");
		test.check(
				!layout.isContextMenuEntryHit(21, entryY + ClientLayout.CONTEXT_MENU_ENTRY_HIT_BOTTOM, 20, 100, entryY),
				"context menu entry strict bottom");
	}
	 ...
	 */
	private record LayoutCase(int width, int height, boolean resizable, int viewportWidth, int viewportHeight,
			int minimapX, int sidebarX, int topTabsX, int bottomTabsX, int chatboxY, int chatModesY,
			int unobscuredWidth, int unobscuredHeight, int centeredX, int centeredY) {
	}
}
