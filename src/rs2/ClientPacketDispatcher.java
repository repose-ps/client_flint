 ...
 *
 * <p>This is the only packet application type allowed to see package-level {@link Client}
 * state. It translates that state into narrow domain bindings and delegates recognized
 * packet effects to {@link PacketDomainDispatcher}.</p>
 */
        ...
        domains = new PacketDomainDispatcher(
                ...
                new PacketDomainDispatcher.RegionBindings(client.regionManager, client.lifecycle::onDemandFetcher,
                        ...
                        client.cameraController, new PacketDomainDispatcher.RegionState() {
                            @Override public int currentPlane() { return client.currentPlane; }
                            @Override public int gameCycle() { return Client.gameCycle; }
                            @Override public int localPlayerServerIndex() { return client.localPlayerServerIndex; }
                            @Override public rs2.game.entity.Player localPlayer() { return Client.localPlayer; }
                            @Override public int destinationX() { return client.destinationX; }
                            @Override public int destinationY() { return client.destinationY; }
                            @Override public void setDestination(int x, int y) {
                                client.destinationX = x;
                                client.destinationY = y;
                            }
                            @Override public void setMultiCombatZone(int value) { client.multiCombatZone = value; }
                        }, (soundId, loops, radius, tileX, tileY) -> client.soundEffectQueue.queueAreaSound(soundId,
                                loops, radius, tileX, tileY, Client.localPlayer.pathX[0], Client.localPlayer.pathY[0],
                                Client.lowMemory), () -> client.drawGameLoadingMessage(null, "Loading - please wait.")),
                new PacketDomainDispatcher.CameraBindings(client.cameraController, () -> client.worldState,
                        () -> client.currentPlane, new PacketDomainDispatcher.HintSink() {
                            @Override public void setType(int type) { client.hintIconType = type; }
                            @Override public void setNpcIndex(int index) { client.hintNpcIndex = index; }
                            @Override public void setTileHint(int tileX, int tileY, int height, int offsetX, int offsetY) {
                            ...
                            @Override public void setPlayerIndex(int index) { client.hintPlayerIndex = index; }
                        }),
                ...
                new PacketDomainDispatcher.ActorBindings(client.actorSynchronizer, client.regionManager,
                        ...
                        value -> client.accountMembershipStatus = value, value -> client.localPlayerServerIndex = value),
                new PacketDomainDispatcher.ClientStateBindings(client.varpState, client.interfaceController,
                        ...
                        client.skillExperiences, client.currentSkillLevels, client.baseSkillLevels, Client.experienceTable,
                        value -> client.systemUpdateTimer = value, value -> client.runEnergy = value));
    }
