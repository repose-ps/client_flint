...
final class SocialPacketHandler {
	 ...
	 */
	SocialPacketHandler(SocialManager social, ChatController chat, IntSupplier tutorialIslandFlag,
			IntSupplier currentWorldId, SocialManager.MessageSink messages,
			PacketDomainDispatcher.AccountInfoSink accountInfo, Runnable redrawChatModes, Runnable redrawChatbox,
			Runnable redrawSidebar) {
		this.social = social;
		this.chat = chat;
	...
	boolean handle(int opcode, Buffer buffer, int packetSize) {
		...
		if (opcode == IncomingPacketOpcode.SERVER_MESSAGE) {
			...
			if (serverMessage.endsWith(":tradereq:")) {
				...
				if (!ignored && tutorialIslandFlag.getAsInt() == 0)
					messages.addChatMessage(duelRequester, "wishes to duel with you.",
							ChatMessageType.CHALLENGE_REQUEST);
		...
		if (opcode == IncomingPacketOpcode.PRIVATE_MESSAGE) {
			...
			if (!duplicateOrIgnored && tutorialIslandFlag.getAsInt() == 0)
				try {
					...
					String privateMessage = ChatCodec.decode(buffer, packetSize - 13);
					if (senderRights != 3)
						privateMessage = Censor.censor(privateMessage);
					if (senderRights == 2 || senderRights == 3)
						messages.addChatMessage(
								"@cr2@" + TextFormatter.formatDisplayName(Base37.decode(senderEncodedName)),
								privateMessage, 7);
					else if (senderRights == 1)
						messages.addChatMessage(
								"@cr1@" + TextFormatter.formatDisplayName(Base37.decode(senderEncodedName)),
								privateMessage, 7);
					else
