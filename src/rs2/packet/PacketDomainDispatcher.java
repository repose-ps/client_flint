...
/**
 * Routes recognized revision-377 application packets to package-private domain handlers.
 *
 * <p>The public facade is intentionally the only packet-domain type visible outside this
 * package. The concrete domain handlers remain package-private implementation details.</p>
 */
public final class PacketDomainDispatcher {
    ...
    public PacketDomainDispatcher(InterfaceBindings interfaces, SocialBindings social, RegionBindings region,
        ...
        regionPackets = new RegionPacketHandler(region.regions(), region.resources(), region.actors(), region.world(),
                region.zoneUpdates(), region.camera(), region.state(), region.areaSounds(), region.showLoadingMessage());
        ...
        clientStatePackets = new ClientStatePacketHandler(clientState.varps(), clientState.interfaces(), clientState.minimap(),
                clientState.applyVarp(), clientState.redrawSidebar(), clientState.redrawChatbox(), clientState.setWeight(),
                clientState.skillExperiences(), clientState.currentSkillLevels(), clientState.baseSkillLevels(),
                clientState.experienceTable(), clientState.setSystemUpdateTimer(), clientState.setRunEnergy());
    }
     ...
     * @return {@code true} when the opcode belongs to a packet domain; {@code false} otherwise
     */
