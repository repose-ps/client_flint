...
final class RegionPacketHandler {
	...
	RegionPacketHandler(RegionManager regions, Supplier<OnDemandFetcher> resources, ActorSynchronizer actors,
			...
			PacketDomainDispatcher.RegionState state, ZoneUpdateHandler.AreaSoundHandler areaSounds,
			Runnable showLoadingMessage) {
	...
	boolean handle(int opcode, Buffer buffer, int packetSize) {
		...
		if (opcode == IncomingPacketOpcode.BATCH_ZONE_UPDATES) {
			zoneUpdates.get().setZoneBase(buffer.readUnsignedByte(), buffer.readUnsignedByteAdd());
			while (buffer.position < packetSize) {
				int updateType = buffer.readUnsignedByte();
				zoneUpdates.get().decode(buffer, updateType, state.currentPlane(), state.gameCycle(),
		...
		if (opcode == IncomingPacketOpcode.REBUILD_REGION || opcode == IncomingPacketOpcode.REBUILD_INSTANCED_REGION) {
			RegionManager.RegionShift shift = regions.decodeRebuild(buffer, opcode, resources.get(), actors,
					world.get(), state.destinationX(), state.destinationY());
			if (shift.changed) {
		...
		if (opcode == IncomingPacketOpcode.PLAY_AREA_SOUND || opcode == IncomingPacketOpcode.UPDATE_GROUND_ITEM_AMOUNT
				|| opcode == IncomingPacketOpcode.ATTACH_OBJECT_TO_PLAYER
				|| opcode == IncomingPacketOpcode.ADD_GROUND_ITEM_FOR_OTHER_PLAYER
				...
				|| opcode == IncomingPacketOpcode.ANIMATE_GAME_OBJECT
				|| opcode == IncomingPacketOpcode.REMOVE_GAME_OBJECT
		...
		if (opcode == IncomingPacketOpcode.SET_ZONE_BASE) {
			zoneUpdates.get().setZoneBase(buffer.readUnsignedByteNeg(), buffer.readUnsignedByteAdd());
			return true;
		}
